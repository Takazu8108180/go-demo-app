...
func (aq *ArticleQuery) QueryComment() *CommentQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, article.CommentTable, article.CommentColumn),
		)
...
func (aq *ArticleQuery) WithComment(opts ...func(*CommentQuery)) *ArticleQuery {
	query := (&CommentClient{config: aq.config}).Query()
...
func (aq *ArticleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Article, error) {
	...
	if query := aq.withComment; query != nil {
		if err := aq.loadComment(ctx, query, nodes,
			func(n *Article) { n.Edges.Comment = []*Comment{} },
			func(n *Article, e *Comment) { n.Edges.Comment = append(n.Edges.Comment, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ArticleQuery) loadComment(ctx context.Context, query *CommentQuery, nodes []*Article, init func(*Article), assign func(*Article, *Comment)) error {
	...
	query.Where(predicate.Comment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(article.CommentColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.article_comment
		if fk == nil {
			return fmt.Errorf(`foreign-key "article_comment" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "article_comment" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
